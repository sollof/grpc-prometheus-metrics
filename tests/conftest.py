    ...
    server = grpc.server(
        ...
        interceptors=(
            PromServerInterceptor(
                legacy=True, enable_handling_time_histogram=True, registry=prom_registry
            ),
        ),
    )
...
def grpc_server_with_exception_handling():
    ...
    server = grpc.server(
        ...
        interceptors=(
            PromServerInterceptor(
                skip_exceptions=True, enable_handling_time_histogram=True, registry=prom_registry
            ),
        ),
    )
    hello_world_grpc.add_GreeterServicer_to_server(Greeter(), server)
...
def grpc_server():
    ...
    server = grpc.server(
        futures.ThreadPoolExecutor(max_workers=2),
        interceptors=(
            PromServerInterceptor(enable_handling_time_histogram=True, registry=prom_registry),
        ),
    )
    hello_world_grpc.add_GreeterServicer_to_server(Greeter(), server)
    server.add_insecure_port("[::]:50051")
...
def grpc_stub():
    ...
    channel = grpc.intercept_channel(
        grpc.insecure_channel("localhost:50051"), PromClientInterceptor(registry=prom_registry)
    )
