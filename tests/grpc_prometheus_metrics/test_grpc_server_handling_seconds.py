...
def test_grpc_server_handling_seconds_with_normal(
    ...
    assert (
        reduce(
            ...
            list(
                filter(
                    lambda x: x.name == "grpc_server_handling_seconds_bucket", target_metric.samples
                )
            ),
            0,
        )
    ...
    assert (
        reduce(
            ...
            list(
                filter(
                    lambda x: x.name == "grpc_server_handling_seconds_count", target_metric.samples
                )
            ),
            0,
        )
    ...
    assert (
        reduce(
            ...
            list(
                filter(
                    lambda x: x.name == "grpc_server_handling_seconds_sum", target_metric.samples
                )
            ),
            0,
        )
...
@pytest.mark.parametrize("number_of_names, number_of_res", [(1, 10), (10, 100), (100, 100)])
def test_grpc_server_handling_seconds_with_bidi_stream(
    ...
    list(grpc_stub.SayHelloBidiStream(bidi_request_generator(number_of_names, number_of_res)))
    target_metric = get_server_metric("grpc_server_handling_seconds")
    assert target_metric.samples == []
